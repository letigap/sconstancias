<?php
        ...
        if(!empty($_POST))
        {
                $valor = $_POST['campo'];
                if(!empty($valor)){
                        $where = "WHERE nombre_evento LIKE '%$valor' order by apellidop";
                }
        }
        $sql = "SELECT apellidop, apellidom, nombre, correo, nombre_evento  FROM participantes  $where";
        //$sql = "SELECT apellidop, apellidom, nombre, correo, nombre_evento  FROM participantes order by nombre_evento";
...
<body>
	<div class="container">
			...
			<form action="<?php
					...
					//	header('Content-Disposition: attachment; filename=asistentes_evento.xls'); 
						$_SERVER['PHP_SELF']; ?>" method="POST">
                                        <b>Nombre: </b><input type="text" id="campo" name="campo" />
                                        <input type="submit" id="enviar" name="enviar" value="Buscar y exportar a Excel" class="btn btn-info" />
